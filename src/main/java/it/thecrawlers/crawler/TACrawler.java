		...
		if (handler.isItemReviewsPage(path) ){
			if (page.getParseData() instanceof HtmlParseData) {
				...
				if(item!=null){
					...
					//Se l'item � "completo" di tutte le recensioni attese
					if (item.getTotalReviewsCount() == item.getReviews().size()){
						...
						//qualunque sia la modalitˆ, qui aggiorniamo il count degli item completati
				...
				//Item non presente, lo aggiungo all'insieme degli item
				{
					...
					//Se l'item appena scoperto � "completo" di tutte le recensioni attese (accade quando l'item ha 10 recensioni o meno)
					if (item.getTotalReviewsCount() == item.getReviews().size()){
